...
import dev.ic2port.blockentity.AbstractCannerBlockEntity;
...
public class VacuumCannerMenu extends MachineWithUpgradesMenu {

    private final AbstractCannerBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public VacuumCannerMenu(final int containerId, final Inventory playerInventory, final FriendlyByteBuf extraData) {
        this(containerId, playerInventory, getBlockEntity(playerInventory, extraData), new SimpleContainerData(4));
    }

    public VacuumCannerMenu(
            final int containerId,
            final Inventory playerInventory,
            final AbstractCannerBlockEntity blockEntity,
            final ContainerData data) {
        super(MenuTypeRegistry.VACUUM_CANNER_MENU.get(), containerId);
        this.blockEntity = blockEntity;
        this.level = playerInventory.player.level();
        this.data = data;

        var itemHandler = blockEntity.getFullItemHandler();

        this.addSlot(createProcessInputSlot(
                itemHandler,
                AbstractCannerBlockEntity.SLOT_TOOL,
                MachineMenuLayout.SLOT_INPUT_X,
                MachineMenuLayout.SLOT_INPUT_Y));
        this.addSlot(createProcessInputSlot(
                itemHandler,
                AbstractCannerBlockEntity.SLOT_SUPPLY,
    ...
    private static AbstractCannerBlockEntity getBlockEntity(
            final Inventory playerInventory,
        ...
        if (blockEntity instanceof AbstractCannerBlockEntity canner) {
            return canner;
        }
        throw new IllegalStateException("Expected vacuum canner block entity at provided BlockPos");
    }
}
