...
public class FoodStorageModuleMenu extends AbstractContainerMenu {
    ...
    public FoodStorageModuleMenu(final int containerId, final Inventory playerInventory, final FriendlyByteBuf extraData) {
        this(containerId, playerInventory, extraData.readVarInt());
    }

    public FoodStorageModuleMenu(final int containerId, final Inventory playerInventory, final int moduleSlot) {
        super(MenuTypeRegistry.FOOD_STORAGE_MODULE_MENU.get(), containerId);
        this.playerInventory = playerInventory;
        this.moduleSlot = moduleSlot;
        ItemStack moduleStack = playerInventory.getItem(moduleSlot);
        if (!(moduleStack.getItem() instanceof FoodStorageModuleItem module)) {
            throw new IllegalStateException("Module slot does not contain a food storage module");
        }
        this.itemHandler = module.createItemHandler(playerInventory, moduleSlot);

        for (int slot = 0; slot < FoodStorageModuleItem.SLOT_COUNT; slot++) {
    ...
    public ItemStack quickMoveStack(final Player player, final int index) {
        ...
        int moduleSlots = FoodStorageModuleItem.SLOT_COUNT;
    ...
    public boolean stillValid(final Player player) {
        ...
        return held.getItem() instanceof FoodStorageModuleItem;
    }
