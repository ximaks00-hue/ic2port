...
public class BatBoxMenu extends AbstractContainerMenu {
    ...
    private final BatBoxBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public BatBoxMenu(final int containerId, final Inventory playerInventory, final FriendlyByteBuf extraData) {
    ...
    public BatBoxMenu(
            ...
            final BatBoxBlockEntity blockEntity,
        ...
        IItemHandler itemHandler = blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER, null)
                .orElseThrow(() -> new IllegalStateException("BatBox item handler capability is missing"));

        this.addSlot(new SlotItemHandler(itemHandler, BatBoxBlockEntity.SLOT_CHARGE, SLOT_CHARGE_X, SLOT_CHARGE_Y) {
            @Override
            public boolean mayPlace(final ItemStack stack) {
                return ItemEnergyHelper.canCharge(stack, BatBoxBlockEntity.TIER);
            }
        });
        this.addSlot(new SlotItemHandler(itemHandler, BatBoxBlockEntity.SLOT_DISCHARGE, SLOT_DISCHARGE_X, SLOT_DISCHARGE_Y) {
            @Override
            public boolean mayPlace(final ItemStack stack) {
                return ItemEnergyHelper.canDischargeInto(stack, BatBoxBlockEntity.TIER);
            }
        });
    ...
    public ItemStack quickMoveStack(final Player player, final int index) {
        ...
        } else if (ItemEnergyHelper.canCharge(sourceStack, BatBoxBlockEntity.TIER)) {
            if (!moveItemStackTo(sourceStack, BatBoxBlockEntity.SLOT_CHARGE, BatBoxBlockEntity.SLOT_CHARGE + 1, false)) {
                return ItemStack.EMPTY;
            }
        } else if (ItemEnergyHelper.canDischargeInto(sourceStack, BatBoxBlockEntity.TIER)) {
            if (!moveItemStackTo(sourceStack, BatBoxBlockEntity.SLOT_DISCHARGE, BatBoxBlockEntity.SLOT_DISCHARGE + 1, false)) {
                return ItemStack.EMPTY;
            }
        } else {
            return ItemStack.EMPTY;
    ...
    private static BatBoxBlockEntity getBlockEntity(final Inventory playerInventory, final FriendlyByteBuf extraData) {
        BlockEntity blockEntity = playerInventory.player.level().getBlockEntity(extraData.readBlockPos());
        if (blockEntity instanceof BatBoxBlockEntity batBox) {
            return batBox;
        }
        throw new IllegalStateException("Expected BatBoxBlockEntity at provided BlockPos");
    }
}
