...
public class MFSUMenu extends AbstractContainerMenu {
    ...
    private final MFSUBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public MFSUMenu(final int containerId, final Inventory playerInventory, final FriendlyByteBuf extraData) {
    ...
    public MFSUMenu(
            ...
            final MFSUBlockEntity blockEntity,
            final ContainerData data) {
        super(MenuTypeRegistry.MFSU_MENU.get(), containerId);
        ...
        this.addSlot(new SlotItemHandler(itemHandler, MFSUBlockEntity.SLOT_CHARGE, SLOT_CHARGE_X, SLOT_CHARGE_Y) {
            ...
            public boolean mayPlace(final ItemStack stack) {
                return ItemEnergyHelper.canCharge(stack, MFSUBlockEntity.TIER);
            }
        });
        this.addSlot(new SlotItemHandler(itemHandler, MFSUBlockEntity.SLOT_DISCHARGE, SLOT_DISCHARGE_X, SLOT_DISCHARGE_Y) {
            @Override
            public boolean mayPlace(final ItemStack stack) {
                return ItemEnergyHelper.canDischargeInto(stack, MFSUBlockEntity.TIER);
            }
        });
    ...
    public ItemStack quickMoveStack(final Player player, final int index) {
        ...
        } else if (ItemEnergyHelper.canCharge(sourceStack, MFSUBlockEntity.TIER)) {
            if (!moveItemStackTo(sourceStack, MFSUBlockEntity.SLOT_CHARGE, MFSUBlockEntity.SLOT_CHARGE + 1, false)) {
                return ItemStack.EMPTY;
            }
        } else if (ItemEnergyHelper.canDischargeInto(sourceStack, MFSUBlockEntity.TIER)) {
            if (!moveItemStackTo(sourceStack, MFSUBlockEntity.SLOT_DISCHARGE, MFSUBlockEntity.SLOT_DISCHARGE + 1, false)) {
                return ItemStack.EMPTY;
            }
        } else {
            return ItemStack.EMPTY;
    ...
    private static MFSUBlockEntity getBlockEntity(final Inventory playerInventory, final FriendlyByteBuf extraData) {
        BlockEntity blockEntity = playerInventory.player.level().getBlockEntity(extraData.readBlockPos());
        if (blockEntity instanceof MFSUBlockEntity mfsu) {
            return mfsu;
        }
        throw new IllegalStateException("Expected MFSUBlockEntity at provided BlockPos");
    }
}
