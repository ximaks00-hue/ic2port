...
public class MaceratorMenu extends MachineWithUpgradesMenu {

    private final MaceratorBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public MaceratorMenu(final int containerId, final Inventory playerInventory, final FriendlyByteBuf extraData) {
        this(containerId, playerInventory, getBlockEntity(playerInventory, extraData), new SimpleContainerData(4));
    }

    public MaceratorMenu(
            final int containerId,
            final Inventory playerInventory,
            final MaceratorBlockEntity blockEntity,
            final ContainerData data) {
        super(MenuTypeRegistry.MACERATOR_MENU.get(), containerId);
        ...
        this.addSlot(new SlotItemHandler(itemHandler, MaceratorBlockEntity.SLOT_OUTPUT, MachineMenuLayout.SLOT_OUTPUT_X, MachineMenuLayout.SLOT_OUTPUT_Y) {
            @Override
    ...
    private static MaceratorBlockEntity getBlockEntity(final Inventory playerInventory, final FriendlyByteBuf extraData) {
        ...
        if (blockEntity instanceof MaceratorBlockEntity maceratorBlockEntity) {
            return maceratorBlockEntity;
        }
        throw new IllegalStateException("Expected MaceratorBlockEntity at provided BlockPos");
    }
}
