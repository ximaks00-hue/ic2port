...
/**
 * Configures teleport tube network ID and send/receive flags.
 */
public class TeleportTubeMenu extends AbstractContainerMenu {
    ...
    public TeleportTubeMenu(final int containerId, final Inventory playerInventory, final FriendlyByteBuf extraData) {
        this(containerId, playerInventory, getBlockEntity(playerInventory, extraData));
    }

    public TeleportTubeMenu(final int containerId, final Inventory playerInventory, final TubeBlockEntity tube) {
        super(MenuTypeRegistry.TELEPORT_TUBE_MENU.get(), containerId);
        this.tube = tube;
        this.data = new ContainerData() {
            @Override
            public int get(final int index) {
                return switch (index) {
                    case 0 -> tube.getTeleportNetworkId();
                    case 1 -> tube.isTeleportSend() ? 1 : 0;
                    case 2 -> tube.isTeleportReceive() ? 1 : 0;
                    default -> 0;
                };
            }
    ...
    public void toggleSend() {
        if (tube.getLevel() != null && !tube.getLevel().isClientSide) {
            tube.toggleTeleportSend();
        }
    }

    public void toggleReceive() {
        if (tube.getLevel() != null && !tube.getLevel().isClientSide) {
            tube.toggleTeleportReceive();
        }
    }

    public void incrementNetworkId() {
        if (tube.getLevel() != null && !tube.getLevel().isClientSide) {
            tube.adjustTeleportNetworkId(1);
        }
    }

    public void decrementNetworkId() {
        if (tube.getLevel() != null && !tube.getLevel().isClientSide) {
            tube.adjustTeleportNetworkId(-1);
        }
    }
    ...
    public boolean stillValid(final Player player) {
        return tube.getRole() == TubeRole.TELEPORT
